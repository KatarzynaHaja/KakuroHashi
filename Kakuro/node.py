...
class Node:
    def __init__(self, number, left, top):
        ...
        self.selected = False
        self.list_of_numbers = {pygame.K_1: 1, pygame.K_2: 2, pygame.K_3: 3,pygame.K_4: 4, pygame.K_5: 5, pygame.K_6: 6,
    ...
    def show(self):
        ...
        t = textObject(str(self.number), 25, self.color)
        rect = t.get_rect()
    ...
    def update(self, event, surface):
        if event.type == pygame.KEYDOWN:
            if self.selected:
                input_entered = self.add_number(event)
                if input_entered:
                    print(input_entered)
