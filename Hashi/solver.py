...
def sort_value(l):
    list_circle = sorted(l, key=lambda circle: (-len(ready(circle.close_neighbors)), circle.value - circle.conections), reverse = True)
    return list_circle
...
def solver(circles):
     ...
     while len(ready(circles)) > 0:
                ...
                if (diff>0 and diff % len(ready(circle[j].close_neighbors)) != 0):
                    ...
                    if iter == 0:
                        while(diff>0):
                            if (diff > 1 and (sort_value(ready(circle[j].close_neighbors))[z].value - sort_value(ready(circle[j].close_neighbors))[z].conections) >= 2):
                                list_of_bridge.append(Bridge(circle[j],sort_value(ready(circle[j].close_neighbors))[z], violet, 2))
                                circle[j].addBridge(sort_value(ready(circle[j].close_neighbors))[z], 2)
                                diff -= 2
                            else:
                                list_of_bridge.append(Bridge(circle[j], sort_value(ready(circle[j].close_neighbors))[z], violet, 1))
                    ...
                    if iter ==1:
                        if diff ==1:
                            list_of_bridge.append(Bridge(circle[j], sort_value(ready(circle[j].close_neighbors))[z+1], violet, 1))
                            circle[j].addBridge(sort_value(ready(circle[j].close_neighbors))[z+1], 1)
                    if diff <=0:
                        print("juz nie mozna")
                        circle[j].is_done =True
